...
app.post("/api/generate-worksheet", async (req, res) => {
  ...
  // Let's build a clear, descriptive prompt for StepUp Math-style generation
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        temperature: 0.3, // Low temperature for consistent math generation
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`StepUp Math Generator Server running on http://localhost:${PORT}`);
  });
}
